...
class LoadROSStreams:  # for inference
    # YOLOv5 local ROS webcam dataloader, i.e. `python detect.py --source 0`
    def __init__(self, pipe='0', img_size=640, stride=32):
        self.img_size = img_size
        self.stride = stride
        self.pipe = eval(pipe) if pipe.isnumeric() else pipe
        self.bridge = CvBridge()
        self.camera_image_topic = '/bebop/image_raw'
        # self.cap = cv2.VideoCapture(self.pipe)  # video capture object
        # self.cap.set(cv2.CAP_PROP_BUFFERSIZE, 3)  # set buffer size
    ...
    def __next__(self):
        ...
        # Read frame
        # ret_val, img0 = self.cap.read()
        raw_img = rospy.wait_for_message(self.camera_image_topic, Image)
        img0 = bridge.imgmsg_to_cv2(raw_img, desired_encoding='passthrough')
        img0 = cv2.flip(img0, 1)  # flip left-right
